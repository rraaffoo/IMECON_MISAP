...
namespace MISAP
{
    public partial class FrmVS_SolicitudDinero : Form  , IForm_opciones //, IForm_filtro
    {
        ...
        public FrmVS_SolicitudDinero(string pproyecto)
        {
            InitializeComponent();
            ts_acciones.Renderer = new MyRenderer();
        ...
        public void pasar_valores(string par1, string par2, string par3, string par4, string par5, string par6)
        {
            try
            {
                ...
                {
                    txtEmpleadoCod.Text = par1;
                    txtEmpleadoDesc.Text = par2;
                }
            }
            catch (Exception ex)
            {
        ...
        private void FrmPedidos_Load(object sender, EventArgs e)
        {
            ...
            cargar_combo_moneda();
            dgvListado.DataSource = Negocio.Listar_Solicitud_Dinero(proyecto);
        ...
        private void pasar_dgv_to_txt(int rownum)
        {
            ...
            txtEmpleadoCod.Text = row.Cells["codEmp"].Value.ToString();
            txtEmpleadoDesc.Text = row.Cells["nomEmp"].Value.ToString();
            txtMonto.Text = row.Cells["monto"].Value.ToString();
            cboMoneda.SelectedValue = row.Cells["monedacod"].Value.ToString() == "SOL" ? "1" : "2";
            txtMotivo.Text = row.Cells["motivo"].Value.ToString();
            txtComentario.Text = row.Cells["comentario"].Value.ToString();
            dtpFecha.Value = Convert.ToDateTime(row.Cells["Fecha"].Value.ToString());
        }
        ...
        //{
        //    cboEmpleado.DataSource = Negocio.Listar_Empleados();
        //    cboEmpleado.ValueMember = "empID";
        //    cboEmpleado.DisplayMember = "fullName";
        //}
        ...
        void formatear_grilla(DataGridView grilla)
        {
            try
            {
                ...
                grilla.Columns["motivo"].Visible = false;
                grilla.Columns["comentario"].Visible = false;
                grilla.Columns["monedacod"].Visible = false;
        ...
        private void cboEmpleado_DrawItem(object sender, DrawItemEventArgs e)
        {
            util.FormatearCombo(sender, e, "fullName");
        }
        ...
        {
            txtEmpleadoCod.BackColor = SystemColors.Window;
            txtEmpleadoDesc.BackColor = SystemColors.Window;
            txtMonto.BackColor = SystemColors.Window;
            txtMotivo.BackColor = SystemColors.Window;
            txtComentario.BackColor = SystemColors.Window;
            
            txtCodigo.Text = "";
            txtEmpleadoCod.Text = "";
            txtEmpleadoDesc.Text = "";
            txtMonto.Text = "";
            txtMotivo.Text = "";
            txtComentario.Text = "";
        ...
        {
            txtEmpleadoCod.BackColor = SystemColors.Control;
            txtEmpleadoDesc.BackColor = SystemColors.Control;
        ...
        {
            configuracion_inicial_controles();
            string mon = cboMoneda.SelectedValue.ToString() == "1" ? "SOL" : "DOL";
            try
            {
                if (flagOperacion == "nuevo")
                {
                    Negocio.VS_SD_RegMod(txtCodigo.Text, dtpFecha.Value, txtMonto.Text, mon, txtMotivo.Text, "", "S", usuario, proyecto, txtComentario.Text, txtEmpleadoCod.Text, txtEmpleadoDesc.Text);
                    dgvListado.DataSource = Negocio.Listar_Solicitud_Dinero(proyecto);
                    formatear_grilla(dgvListado);
        ...
        {
            util.validar_numeros(txtMonto, e);
        }
        ...
        private void btnSuprimir_Click(object sender, EventArgs e)
        {
            ...
            if (resul == DialogResult.Yes)
            {
                try
                {
                    Negocio.VS_SD_RegMod(txtCodigo.Text, dtpFecha.Value, txtMonto.Text, mon, txtMotivo.Text, "", "X", usuario, proyecto, txtComentario.Text, txtEmpleadoCod.Text, txtEmpleadoDesc.Text);
                    dgvListado.DataSource = Negocio.Listar_Solicitud_Dinero(proyecto);
                    formatear_grilla(dgvListado);
        ...
        {
            configuracion_inicial_controles();
            mon = cboMoneda.SelectedValue.ToString() == "1" ? "SOL" : "DOL";
        //    string mon = cboMoneda.SelectedValue.ToString() == "1" ? "SOL" : "DOL";
        //    //DialogResult resul = MessageBox.Show("¿Está seguro que quiere RECHAZAR esta Solicitud?", "Rechazar Proyecto", MessageBoxButtons.YesNo);
        //    //if (resul == DialogResult.Yes)
        //    //{
        //        try
        //        {
        //            Negocio.VS_SD_RegMod(txtCodigo.Text, dtpFecha.Value, txtMonto.Text, mon, txtMotivo.Text, "", "R", "ASIALM06", proyecto, txtComentario.Text, txtEmpleadoCod.Text, txtEmpleadoDesc.Text);
        //            dgvListado.DataSource = Negocio.Listar_Solicitud_Dinero(proyecto);
        //            formatear_grilla(dgvListado);
        //            util.mensaje("Se RECHAZÓ la Solicitud de manera exitosa", true, lbl_contador_registros, lbl_msg, ss_load, t_msg);
        //        }
        //        catch (Exception ex)
        //        {
        //            util.mensaje("Error: " + ex.Message, false, lbl_contador_registros, lbl_msg, ss_load, t_msg);
        ...
        //    }
        //}
            FrmVS_Motivo_Anulacion motivoAnulacion = new FrmVS_Motivo_Anulacion();
            motivoAnulacion.pCodigo = txtCodigo.Text;
            motivoAnulacion.pFecha = dtpFecha.Value;
            motivoAnulacion.pMonto = txtMonto.Text;
            motivoAnulacion.pMoneda = mon;
            motivoAnulacion.pMotivo = txtMotivo.Text;
            motivoAnulacion.pMotivoAdic = "";
            motivoAnulacion.pEstado = "R";
            motivoAnulacion.pUsuario = usuario;
            motivoAnulacion.pProyecto = proyecto;
            motivoAnulacion.pComentario = txtComentario.Text;
            motivoAnulacion.pEmpleadoCod = txtEmpleadoCod.Text;
            motivoAnulacion.pEmpleadoDesc = txtEmpleadoDesc.Text;
            motivoAnulacion.FormClosed += new FormClosedEventHandler(ingMasivo_FormClosed);
            motivoAnulacion.ShowDialog();
        }
        ...
        private void btnAprobar_Click(object sender, EventArgs e)
        {
            configuracion_inicial_controles();
            mon = cboMoneda.SelectedValue.ToString() == "1" ? "SOL" : "DOL";
            DialogResult resul = MessageBox.Show("¿Está seguro que quiere APROBAR esta Solicitud?", "Aprobar Proyecto", MessageBoxButtons.YesNo);
            if (resul == DialogResult.Yes)
            {
                try
                {
                    Negocio.VS_SD_RegMod(txtCodigo.Text, dtpFecha.Value, txtMonto.Text, mon, txtMotivo.Text, "", "A", usuario, proyecto, txtComentario.Text, txtEmpleadoCod.Text, txtEmpleadoDesc.Text);
                    dgvListado.DataSource = Negocio.Listar_Solicitud_Dinero(proyecto);
                    formatear_grilla(dgvListado);
                    util.mensaje("Se APROBÓ la Solicitud de manera exitosa", true, lbl_contador_registros, lbl_msg, ss_load, t_msg);
                }
                catch (Exception ex)
                {
                    util.mensaje("Error: " + ex.Message, false, lbl_contador_registros, lbl_msg, ss_load, t_msg);
