...
namespace MISAP
{
    public partial class FrmVS_GestionCajaChica : Form  //, IForm_opciones, IForm_filtro
    {
        ...
        public FrmVS_GestionCajaChica(string pproyecto)
        {
            InitializeComponent();
            ts_acciones.Renderer = new MyRenderer();
        ...
        private void FrmPedidos_Load(object sender, EventArgs e)
        {
            ...
            cargar_combo_estado();
            dgvListado.DataSource = Negocio.VS_Listar_CajaChica(proyecto);
        ...
        private void pasar_dgv_to_txt(int rownum)
        {
            ...
            txtCaja.Text = row.Cells["NRO CAJA"].Value.ToString();
            txtTotalCaja.Text = row.Cells["TOTAL CAJA"].Value.ToString();
            cboMoneda.SelectedValue = row.Cells["COD_MONEDA"].Value.ToString()=="SOL"? "1" : "2";
            txtOT.Text = row.Cells["NRO OT"].Value.ToString();
            cboEstado.SelectedValue = row.Cells["COD_ESTADO"].Value.ToString();
            dtpFecha.Value = Convert.ToDateTime(row.Cells["FECHA CIERRE"].Value.ToString() =="" ? "01/01/1990" : row.Cells["FECHA CIERRE"].Value.ToString());
        ...
        void formatear_grilla(DataGridView grilla)
        {
            try
            {
                ...
                grilla.Columns["COD_MONEDA"].Visible = false;
                grilla.Columns["COD_ESTADO"].Visible = false;
                grilla.Columns["COD_CONTABILIZADO"].Visible = false;
        ...
        {
            FrmVS_CajaChica_RegistroGastos Gasto = new FrmVS_CajaChica_RegistroGastos(txtdocEntry.Text, txtCaja.Text, txtTotalCaja.Text, cboMoneda.SelectedValue.ToString(), txtOT.Text, cboEstado.SelectedValue, dtpFecha.Value.ToShortDateString());
            Gasto.ShowDialog();
        }
        ...
        private void btnGrabar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                Negocio.VS_modificar_caja_chica(txtdocEntry.Text, txtCaja.Text, txtOT.Text, Convert.ToDecimal(txtTotalCaja.Text), mon, dtpFecha.Value, cboEstado.SelectedValue.ToString(), "ASIALM06", proyecto, txtComentario.Text, 0);
                dgvListado.DataSource = Negocio.VS_Listar_CajaChica(proyecto);
                formatear_grilla(dgvListado);
